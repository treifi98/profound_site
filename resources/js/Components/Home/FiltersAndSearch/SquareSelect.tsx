...
const SquareSelect = (props:Props) => {
    ...
    const handleButtonClick = (index:number,val:string) => {
        ...
        if(elmnt.classList.value.includes('clicked')){
            ...
            setClickedIndexes((prev) => [String(index)])
        }
    }
    useEffect(()=>{
  ...
  return (
    <div className='select-none cursor-pointer group w-[9rem] h-[2.25rem] flex border-[0.06rem] border-[#DCE3E3] rounded-[3.13rem] relative' onClick={handleClick} ref={componentRef}>
        <div className='w-[5.75rem] h-[2.25rem] rounded-[3.13rem_0rem_0rem_3.13rem] bg-[#E6EDED]  flex justify-center items-center shadow-[-0.285380625rem_-0.285380625rem_0.456609375rem_#FFFFFF,0.285380625rem_0.285380625rem_0.399533125rem_#B6C3C5]' >
            <div className='text-grade'>
                {props.lable}

            </div>
        </div>
        <div className=' relative w-[3.0625rem] group-hover:shadow-[inset_-0.25rem_-0.25rem_0.3125rem_#FFFFFF,inset_0.285380625rem_0.285380625rem_0.399533125rem_#B6C3C5] h-[2.25rem] bg-[#E6EDED] flex justify-center items-center shadow-[0rem_-0.285380625rem_0.456609375rem_#FFFFFF,0.285380625rem_0.285380625rem_0.399533125rem_#B6C3C5] rounded-[0rem_3.13rem_3.13rem_0rem]' ref={selectHandleRef}>
        ...
        </div>
        {
            ...
            <div className='absolute w-[9rem] h-[10.375rem] top-[2.25rem] left-[0rem] rounded-[0.63rem] z-[999999]'>
                <div className='w-[9rem] h-max py-[0.5625rem] bg-[#E6EDED] top-[0rem] mt-[0.875rem] left-[0rem] border-[0.06rem] rounded-[0.63rem] shadow-[-0.285380625rem_-0.285380625rem_0.456609375rem_#FFFFFF,0.285380625rem_0.285380625rem_0.399533125rem_#B6C3C5] flex justify-center items-center ' onClick={(e) => e.stopPropagation()}>
                    {
                        <div className='w-[90%] h-max flex flex-wrap gap-x-[0.4375rem] justify-center gap-y-[0.625rem] 'style={openState ? mountedStyle : unmountedStyle}>
                            {
                                props.options.map((option, index) => (
                                    <div className='w-[3.4375rem] h-[1.5625rem] flex justify-center items-center rounded-[0.50rem] bg-[#E6EDED] ' onClick={() => handleButtonClick(index,option.value)} key={option.value} ref={refs[index]} style={{ boxShadow:'-0.15865375rem -0.15865375rem 0.264423125rem #FFFFFF, 0.264423125rem 0.264423125rem 0.3701925rem #B6C3C5' }}>
                                        <div style={{fontSize:'0.75rem',fontWeight:'600'} } className='text-grade font-[600]' key={option.value}>
